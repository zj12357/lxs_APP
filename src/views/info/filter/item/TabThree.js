...
import {getSportFilterThree, commitSportFilterThree} from '@/store/actions';
import _ from 'lodash';
import {withNavigation} from 'react-navigation';
const mapStateToProps = state => {
  return {
    sportFilterThree: state.info.sportFilterThree,
  };
};

const mapDispatchToProps = {
  getSportFilterThree,
  commitSportFilterThree,
};
@connect(
  mapStateToProps,
  mapDispatchToProps,
...
export default class Filter extends Component {
  ...
  render() {
    let data = _.cloneDeep(this.props.sportFilterThree);
    data = _.flatten(data);
      ...
      <View tabLabel="全部赛事" style={style.tabOne}>
        {data.length ? (
          <FlatList
            ...
            data={this.props.sportFilterThree}
  ...
  componentDidMount = async () => {
    let params = {level: 1};
    await this.props.getSportFilterThree(params);
  };
  handleItem(index, i) {
    let data = _.cloneDeep(this.props.sportFilterThree);
    data[index][i].icon = !data[index][i].icon;
    this.props.commitSportFilterThree(data);
  }
  handleAll = () => {
    let data = _.cloneDeep(this.props.sportFilterThree);
    data.forEach(item => {
